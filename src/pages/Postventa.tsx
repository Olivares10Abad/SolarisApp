...
import ModalPostventaDetalle from '../components/ModalPostventaDetalle'
import ModalCalendarioPostventa from '../components/ModalCalendarioPostventa'
...
export default function Postventa() {
   ...
   const [postventaes, setPostventaes] = useState<any[]>([])
   ...
   const fetchPostventaes = async () => {
      setCargando(true)
      const { data } = await supabase
         .from('postventa_control')
         .select(`
        *,
   ...
   const postventaesFiltradas = useMemo(() => {
      return postventaes.filter(v => {
         const p = v.proyecto || {};
   ...
   }, [postventaes, busqueda, filtroPaso]);
   ...
   const agendarVisita = async () => {
      ...
      try {
         ...
         await supabase.from('postventa_control').update(updates).eq('id', proyectoSeleccionado.id)
         ...
         await supabase.from('proyectos_interacciones').insert([{
            ...
            estado_anterior: 'Postventa',
            estado_nuevo: 'Postventa',
            accion: 'Agenda Postventa',
         ...
         await enviarNotificacionVendedor(
            ...
            `📅 Tu visita de Postventa ha sido agendada para el ${agendaForm.fecha_inicio} a las ${agendaForm.hora_inicio}.|||/proyectos?proyecto_id=${proyectoSeleccionado.proyecto_id}`,
            usuarioLogueado?.id
         );
         ...
         if (agendaForm.ingeniero_id) {
            await enviarNotificacionVendedor(
               ...
               `🛠️ Se te ha asignado una postventa técnica para el ${agendaForm.fecha_inicio} a las ${agendaForm.hora_inicio}: ${proyectoSeleccionado.proyecto?.nombre_proyecto}|||/postventa?proyecto_id=${proyectoSeleccionado.proyecto_id}`,
               usuarioLogueado?.id
            );
         }

         setShowModalSecundario(null);
         await fetchPostventaes();
   ...
   const confirmarCancelacion = async () => {
      ...
      setProcesando(true);
      try {
         await supabase.from('postventa_control').update({
            status: 0,
         ...
         await supabase.from('proyectos_interacciones').insert([{
            ...
            accion: 'Rechazo',
            mensaje: `Se rechazó la solicitud de postventa técnica y se retornó a evaluación. Motivo: ${motivoCancelacion}`
         }]);

         await enviarNotificacionVendedor(
            proyectoSeleccionado.proyecto?.vendedor_id, 
            `❌ Tu solicitud de Postventa Técnica ha sido rechazada. Motivo: ${motivoCancelacion}|||/evaluacion?proyecto_id=${proyectoSeleccionado.proyecto_id}`, 
            usuarioLogueado?.id
         );
   ...
   const avanzarA = async (targetStatus: number, uploadPdf: boolean = false) => {
      ...
      try {
         ...
         if (targetStatus === 2) {
            ...
            await supabase.from('proyectos').update({ estatus: 'Postventa', sub_estatus: 'Pendiente Aprobacion Ventas' }).eq('id', proyectoSeleccionado.proyecto_id);
            ...
            await supabase.from('proyectos_interacciones').insert([{
               ...
               mensaje: 'Ingeniería aceptó la solicitud de postventa. Esperando aprobación de presupuesto de Ventas.'
            }]);

            await enviarNotificacionVendedor(proyectoSeleccionado.proyecto?.vendedor_id, `⚙️ Ingeniería aceptó tu Solicitud de Postventa. Requiere que apruebes el proyecto en la bandeja de Aprobaciones.`, usuarioLogueado?.id);
         } else if (targetStatus === 7) {
            await supabase.from('proyectos').update({ estatus: 'Postventa - Revisión', sub_estatus: null }).eq('id', proyectoSeleccionado.proyecto_id);
            await enviarNotificacionRoles('notif_postventa_revision', `Postventa Técnica finalizada, requiere revisión gerencial: ${proyectoSeleccionado.proyecto?.nombre_proyecto}|||/revision`, usuarioLogueado?.id);
         } else {
            ...
            if (targetStatus === 4) {
               await enviarNotificacionVendedor(proyectoSeleccionado.proyecto?.vendedor_id, `✅ Tu solicitud de Postventa Técnica ha sido verificada y confirmada en agenda: ${proyectoSeleccionado.proyecto?.nombre_proyecto}|||/proyectos?proyecto_id=${proyectoSeleccionado.proyecto_id}`, usuarioLogueado?.id);
            }
         }
         ...
         await showAlert('Aviso', 'Error al avanzar postventa.')
      } finally {
         setProcesando(false)
      }
   }
               ...
               {cargando ? (
                  ...
                  <div className="flex-1 flex items-center justify-center opacity-70">
                     <div className="bg-white/60 backdrop-blur-md p-10 rounded-3xl border border-white/40 shadow-xl text-center">
                        ...
                        <p className="text-sm font-bold text-slate-500 mt-2">No se encontraron postventaes.</p>
                     </div>
                  </div>
               ) : (
                  <div className="flex flex-col gap-4">
                     {postventaesFiltradas.map((v) => {
            ...
            <ModalPostventaDetalle
               ...
               onChatClick={() => {
                  setChatInicial({
                     ...
                     nombre: proyectoSeleccionado.proyecto?.nombre_proyecto || 'Postventa',
                     estatusFiltro: 'Postventa',
                     estatusProyecto: 'Postventa',
               ...
               <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white border-[3px] border-slate-100 w-full max-w-[420px] rounded-[32px] overflow-hidden shadow-2xl relative">
                     <div className="bg-slate-50 px-5 py-4 flex justify-between items-center border-b border-slate-100">
                        <h3 className="font-black tracking-widest uppercase text-slate-800 text-[11px] flex items-center gap-2"><Info size={16} /> Info Solicitud de Postventa</h3>
               ...
               <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[#f0f2f5] border-[3px] border-white w-full max-w-[340px] rounded-[32px] overflow-hidden shadow-2xl relative">
                     <div className="bg-white px-5 py-4 flex justify-between items-center border-b-[4px] border-red-500">
                        <h3 className="font-black tracking-widest uppercase text-slate-800 text-[11px]">Rechazar Postventa</h3>
                        <button onClick={() => setShowModalSecundario(null)} className="p-1 text-slate-400 hover:text-red-500"><X size={26} strokeWidth={2.5} /></button>
                     </div>
               ...
               <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[#f0f2f5] border-[3px] border-white w-full max-w-[320px] rounded-[32px] overflow-hidden shadow-2xl relative">
                     ...
                     <div className="p-6 text-center flex flex-col gap-5 bg-white">
                        ...
                        <p className="text-xs font-bold text-slate-600">¿Estás seguro de confirmar la cita de postventa técnica agendada?</p>
                  ...
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-[#f0f2f5] border-[3px] border-white w-full max-w-[420px] rounded-[32px] overflow-hidden shadow-2xl relative">
                     ...
                     <div className="p-6">
                        <div className="bg-white rounded-3xl p-6 border border-slate-100 mb-6 space-y-6 shadow-sm">
                           ...
                           {/* Inicio */}
                           <div>
                              <label className="block text-[11px] font-black uppercase tracking-widest text-slate-400 mb-3 ml-1">Fecha Postventa Inicio</label>
                           ...
                           {/* Fin */}
                           <div>
                              <label className="block text-[11px] font-black uppercase tracking-widest text-slate-400 mb-3 ml-1">Fecha Postventa Fin</label>
                              <div className="flex border-2 border-slate-900 rounded-xl overflow-hidden w-full bg-white relative hover:border-[#ffb000] focus-within:border-[#ffb000] transition-colors">
                                 <input type="datetime-local" 
