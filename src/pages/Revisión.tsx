                ...
                <div className="p-6 md:p-8 bg-white flex flex-col gap-5 md:gap-6">
                  ...
                  {(proyectoSeleccionado.estatus === 'Cotización – Revisión') && (
                    <div className="grid grid-cols-2 gap-3 pt-4 border-t border-slate-100 mt-1">
                      ...
                      <button onClick={() => setModalAprobar(true)} className="py-3 md:py-4 rounded-xl md:rounded-2xl border-2 border-emerald-500 bg-emerald-500 text-white font-black text-[9px] md:text-[11px] uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-emerald-200 hover:bg-emerald-600 transition-colors"> <CheckCircle2 className="w-4 h-4 md:w-5 md:h-5"/> Aprobar (Cotizado) </button>
                    </div>
                  )}
                </div>
              </motion.div>
                     ...
                     <div className="bg-slate-900 p-5 md:p-6 flex justify-between items-center text-white shrink-0 border-b border-white/10">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-[8px] md:text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1 truncate max-w-[150px] md:max-w-full">{proyectoSeleccionado.nombre_proyecto}</p>
                            </div>
                        </div>
                        <button onClick={() => setModalLog(false)} className="p-2 bg-white/10 hover:bg-red-500 rounded-full transition-colors"><X className="w-4 h-4 md:w-5 md:h-5"/></button>
                     </div>
                                     ...
                                     return (
                                        <div key={log.id} className="bg-white p-3 md:p-4 rounded-xl border border-slate-100 flex gap-3 md:gap-4 relative overflow-hidden shadow-sm">
                                             ...
                                             <div className="flex-1 text-[11px]">
                                                 ...
                                                 {log.mensaje && <div className="p-2.5 md:p-3 bg-white text-slate-700 rounded-xl mt-2 md:mt-3 italic border-l-4 border-blue-400 font-medium text-[9px] md:text-[11px] shadow-inner whitespace-pre-line">{log.mensaje}</div>}
                                             </div>
                                        </div>
                                     )
                                })}
