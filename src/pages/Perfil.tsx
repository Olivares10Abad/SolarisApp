...
export default function Perfil() {
    ...
    const [tabActiva, setTabActiva] = useState<'informacion' | 'documentos' | 'vacaciones' | 'notificaciones'>('informacion')
    ...
    const toggleNotificacion = async (campo: string) => {
        if (!perfil) return;
        const nuevoValor = !perfil[campo];
        const { error } = await supabase.from('perfiles').update({ [campo]: nuevoValor }).eq('id', perfil.id);
        if (!error) {
            setPerfil({ ...perfil, [campo]: nuevoValor });
            const updatedSession = { ...sessionUser, [campo]: nuevoValor };
            localStorage.setItem('session_gea_solar', JSON.stringify(updatedSession));
            setSessionUser(updatedSession);
        }
    }
                    ...
                    <section className="col-span-1 lg:col-span-8 flex flex-col">
                        ...
                        <div className="overflow-x-auto pb-4 mb-4 custom-scrollbar shrink-0">
                            <div className="flex bg-white/95 backdrop-blur-xl p-1.5 rounded-[20px] md:rounded-[24px] shadow-lg border border-white w-max min-w-full md:min-w-fit md:w-fit">
                                ...
                                <button onClick={() => setTabActiva('notificaciones')} className={`flex-1 md:flex-none px-4 md:px-6 py-2.5 md:py-3 rounded-[16px] md:rounded-[18px] text-[9px] md:text-[10px] font-black transition-all flex items-center justify-center gap-2 uppercase tracking-widest whitespace-nowrap ${tabActiva === 'notificaciones' ? 'bg-slate-900 text-white shadow-xl' : 'text-slate-500 hover:text-slate-900'}`}><BellRing className="w-3.5 h-3.5 md:w-4 md:h-4" /> Alertas</button>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 min-h-0">
                            <AnimatePresence mode='wait'>
                                ...
                                {/* --- PESTAÑA 4: CONFIGURACIÓN DE ALERTAS --- */}
                                {tabActiva === 'notificaciones' && (
                                    <motion.div key="notif" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-white/95 backdrop-blur-xl rounded-[30px] md:rounded-[40px] p-6 md:p-8 shadow-2xl border border-white">
                                        <h3 className="text-base md:text-lg font-black uppercase tracking-widest text-slate-900 border-b border-slate-100 pb-3 md:pb-4 mb-4 md:mb-6 flex items-center gap-3"><BellRing className="w-5 h-5 md:w-6 md:h-6 text-orange-500" /> Centro de Notificaciones</h3>
                                        <p className="text-[10px] md:text-xs font-bold text-slate-500 mb-6 uppercase tracking-widest">Sincroniza tus alertas push para los eventos críticos de la plataforma.</p>
