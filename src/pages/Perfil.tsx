      ...
      <nav className="bg-white/95 backdrop-blur-2xl border-b border-white/20 sticky top-0 z-50 shadow-lg h-16 flex items-center relative">
        <div className="max-w-[1800px] mx-auto px-6 w-full flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-black text-lg tracking-tight text-slate-900 uppercase italic">Gestión de Perfil</h1>
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-[1800px] mx-auto px-6 py-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <aside className="col-span-1 lg:col-span-4 space-y-6">
                <div className="bg-white/95 backdrop-blur-xl rounded-[40px] p-8 shadow-2xl border border-white text-center relative overflow-hidden">
                    <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-br from-yellow-400 to-orange-500 opacity-20"></div>
                    
                    <div className="relative w-32 h-32 mx-auto mt-4 mb-6 group">
                        <div className="w-full h-full rounded-3xl bg-slate-900 text-white flex items-center justify-center shadow-xl overflow-hidden border-4 border-white relative z-10">
                            {subiendoAvatar ? <Loader2 className="w-8 h-8 animate-spin text-orange-500"/> : getAvatar()}
                        </div>
                        <div onClick={() => fileInputRef.current?.click()} className="absolute inset-0 bg-slate-900/60 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white cursor-pointer z-20 backdrop-blur-sm border-4 border-transparent">
                    ...
                    <h2 className="text-2xl font-black text-slate-900 uppercase leading-none">{perfil?.nombre} {perfil?.apellidos}</h2>
                    <p className="text-[10px] font-black text-orange-600 bg-orange-50 px-3 py-1 rounded-md uppercase tracking-widest border border-orange-100 w-fit mx-auto mt-3">{perfil?.rol_sistema}</p>
                    
                    <div className="mt-8 space-y-4 text-left border-t border-slate-100 pt-6">
                        <div className="flex items-center gap-3 text-slate-600"><Briefcase className="w-4 h-4 text-slate-400"/><div className="text-xs"><p className="font-bold text-slate-900 uppercase">Departamento</p><p className="text-[10px] font-bold uppercase tracking-widest">{perfil?.departamento}</p></div></div>
                        <div className="flex items-center gap-3 text-slate-600"><Network className="w-4 h-4 text-slate-400"/><div className="text-xs"><p className="font-bold text-slate-900 uppercase">Jefe Directo</p><p className="text-[10px] font-bold uppercase tracking-widest">{perfil?.jefe?.nombre ? `${perfil.jefe.nombre} ${perfil.jefe.apellidos}` : 'Nivel Directivo'}</p></div></div>
                        <div className="flex items-center gap-3 text-slate-600"><Calendar className="w-4 h-4 text-orange-500"/><div className="text-xs"><p className="font-bold text-slate-900 uppercase">Fecha de Ingreso</p><p className="text-[10px] font-black text-orange-600 uppercase tracking-widest">{perfil?.fecha_ingreso ? new Date(perfil.fecha_ingreso).toLocaleDateString('es-MX', {year: 'numeric', month: 'long', day: 'numeric'}) : 'No registrada'}</p></div></div>
                    </div>
                </div>
            </aside>
            ...
            <section className="col-span-1 lg:col-span-8 space-y-6">
                ...
                <div className="flex bg-white/95 backdrop-blur-xl p-1.5 rounded-[24px] shadow-lg border border-white w-fit">
                    <button onClick={() => setTabActiva('informacion')} className={`px-6 py-3 rounded-[18px] text-[10px] font-black transition-all flex items-center gap-2 uppercase tracking-widest ${tabActiva === 'informacion' ? 'bg-slate-900 text-white shadow-xl' : 'text-slate-500 hover:text-slate-900'}`}><Fingerprint className="w-4 h-4" /> Información</button>
                    <button onClick={() => setTabActiva('documentos')} className={`px-6 py-3 rounded-[18px] text-[10px] font-black transition-all flex items-center gap-2 uppercase tracking-widest ${tabActiva === 'documentos' ? 'bg-slate-900 text-white shadow-xl' : 'text-slate-500 hover:text-slate-900'}`}><FileBadge className="w-4 h-4" /> Expediente</button>
                    <button onClick={() => setTabActiva('vacaciones')} className={`px-6 py-3 rounded-[18px] text-[10px] font-black transition-all flex items-center gap-2 uppercase tracking-widest ${tabActiva === 'vacaciones' ? 'bg-slate-900 text-white shadow-xl' : 'text-slate-500 hover:text-slate-900'}`}><PlaneTakeoff className="w-4 h-4" /> Vacaciones</button>
                </div>
                ...
                <AnimatePresence mode='wait'>
                    ...
                    {tabActiva === 'informacion' && (
                        <motion.div key="info" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-white/95 backdrop-blur-xl rounded-[40px] p-8 shadow-2xl border border-white">
                            ...
                            <form onSubmit={guardarInformacion} className="space-y-6">
                                ...
                                <div className="pt-4 flex justify-end">
                                    <button type="submit" disabled={guardandoInfo} className="bg-slate-900 text-white px-12 py-4 rounded-[20px] font-black text-xs hover:bg-purple-600 transition-all shadow-xl disabled:opacity-50 uppercase tracking-widest flex items-center gap-3">
                                        {guardandoInfo ? <Loader2 className="w-4 h-4 animate-spin"/> : <><Save className="w-4 h-4"/> Guardar Cambios</>}
                                    </button>
                                </div>
                            </form>
                        </motion.div>
                    ...
                    {tabActiva === 'vacaciones' && (
                        <motion.div key="vacaciones" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
                            ...
                            {!perfil?.fecha_ingreso ? (
                                <div className="bg-orange-50 border border-orange-200 rounded-[30px] p-6 text-center text-orange-700 font-bold text-sm">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div className="bg-white/95 backdrop-blur-xl border border-slate-100 shadow-xl rounded-[35px] p-8 text-center relative overflow-hidden">
                                        <div className="absolute -top-4 -right-4 w-24 h-24 bg-blue-500 rounded-full blur-3xl opacity-10 pointer-events-none"></div>
                                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Por Ley (LFT)</p>
                                        <p className="text-5xl font-black italic text-slate-900">{resumenVacaciones.total}</p>
                                        <p className="text-[9px] font-black uppercase text-blue-500 mt-2 bg-blue-50 py-1 rounded-md mx-4">{resumenVacaciones.años} años Antigüedad</p>
                                    </div>
                                    <div className="bg-white/95 backdrop-blur-xl border border-slate-100 shadow-xl rounded-[35px] p-8 text-center relative overflow-hidden">
                                        <div className="absolute -top-4 -right-4 w-24 h-24 bg-orange-500 rounded-full blur-3xl opacity-10 pointer-events-none"></div>
                                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Utilizados</p>
                                        <p className="text-5xl font-black italic text-slate-900">{resumenVacaciones.tomados}</p>
                                        <p className="text-[9px] font-black uppercase text-orange-500 mt-2 bg-orange-50 py-1 rounded-md mx-4">En ciclo anual</p>
                                    </div>
                                    <div className="bg-white/95 backdrop-blur-xl border border-slate-100 shadow-xl rounded-[35px] p-8 text-center relative overflow-hidden">
                                        <div className="absolute -top-4 -right-4 w-24 h-24 bg-emerald-500 rounded-full blur-3xl opacity-10 pointer-events-none"></div>
                                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Disponibles</p>
                                        <p className="text-5xl font-black italic text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-emerald-600">{resumenVacaciones.restantes}</p>
                                        <p className="text-[9px] font-black uppercase text-emerald-600 mt-2 bg-emerald-50 py-1 rounded-md mx-4">Listos para usar</p>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                                ...
                                <div className="lg:col-span-7 bg-white/95 backdrop-blur-xl rounded-[40px] p-8 shadow-2xl border border-white h-fit">
                                    ...
                                    <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 mb-6">
                                        <div className="flex items-center justify-between mb-6 px-2">
                                            <button type="button" onClick={() => setPickerMonth(new Date(pickerMonth.getFullYear(), pickerMonth.getMonth() - 1, 1))} className="p-1.5 hover:bg-slate-200 rounded-lg text-slate-400 hover:text-orange-500 transition-colors"><ChevronLeft className="w-5 h-5"/></button>
                                            <h3 className="text-sm font-black uppercase tracking-widest text-slate-900">{mesesNombres[pickerMonth.getMonth()]} {pickerMonth.getFullYear()}</h3>
                                            <button type="button" onClick={() => setPickerMonth(new Date(pickerMonth.getFullYear(), pickerMonth.getMonth() + 1, 1))} className="p-1.5 hover:bg-slate-200 rounded-lg text-slate-400 hover:text-orange-500 transition-colors"><ChevronRight className="w-5 h-5"/></button>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-7 gap-1 relative">
                                            {Array.from({ length: diaInicioMes }).map((_, i) => <div key={`empty-${i}`} className="h-10" />)}
                                    ...
                                    <form onSubmit={handleSolicitarVacaciones} className="space-y-6">
                                ...
                                <div className="lg:col-span-5 bg-white/95 backdrop-blur-xl rounded-[40px] p-8 shadow-2xl border border-white flex flex-col max-h-[750px]">
                                    ...
                                    <div className="overflow-y-auto pr-2 space-y-4 custom-scrollbar flex-1">
                                        {solicitudes.length === 0 ? <p className="text-xs text-slate-400 font-bold text-center py-10">No hay solicitudes.</p> : solicitudes.map(sol => (
                                            <div key={sol.id} className="bg-slate-50 border border-slate-100 rounded-3xl p-5 relative shadow-sm">
                                                <div className="flex justify-between items-start mb-3">
                                                    <div>
                                                        <p className="text-sm font-black text-slate-900 uppercase">{sol.dias_solicitados} Días</p>
                                                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">{sol.fecha_inicio} al {sol.fecha_fin}</p>
                                                    </div>
                                                    <span className={`text-[8px] font-black px-3 py-1.5 rounded-md uppercase tracking-widest border
                                                        ...
                                                        {sol.estado === 'Aprobada' && <CheckCircle2 className="w-3 h-3 inline mr-1.5 -mt-0.5"/>}
                                                        {sol.estado === 'Rechazada' && <XCircle className="w-3 h-3 inline mr-1.5 -mt-0.5"/>}
                                                        {sol.estado}
                                                    </span>
                                                </div>
                                                {sol.motivo && <p className="text-xs text-slate-600 font-medium italic mt-2">"{sol.motivo}"</p>}
                                                {sol.estado === 'Pendiente' && <button onClick={() => cancelarSolicitud(sol.id)} className="text-[10px] font-black text-red-500 uppercase hover:underline mt-4 block text-right w-full">Cancelar</button>}
                                            </div>
                                        ))}
                                    </div>
                                </div>
